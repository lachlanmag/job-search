...
| Placeholder | Source |
|-------------|--------|
| `{job_description}` | Full JD text: user paste, local file path, or saved JD the user names |
...
**Tracker shortcut:** If the user names a shortlisted role, read `data/applications.yaml` for company/title context only. Still require the JD and tailored resume JSON (from paths the user provides).
...
Use a lowercase slug from company name (e.g. `acme-corp`). If multiple reviews same day, append `-2`, `-3`, etc.
...
- No em dashes in generated report text (per prompt).
