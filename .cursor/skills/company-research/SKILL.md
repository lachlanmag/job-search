...
| User `jd_path` on tracker row | Saved JD markdown if present |
...
- Skip if `company_research` on the row points to an artifact dated today for the same role (idempotent re-run). Otherwise proceed.
...
`data/company-research/YYYY-MM-DD-{company-slug}.md`

Lowercase slug from company name (e.g. `acme-corp`). Same day + same company: append `-2`, `-3`, etc.

### 5. Update tracker

On the application row, set:

- `company_research: data/company-research/YYYY-MM-DD-{company-slug}.md`
...
3. Note artifact path and remind: save JD locally if not already (`jd_path`), then tailor resume and run `resume-feedback` before applying.
